...
)
{
	...
	switch (grouping_set->kind)
	{
		...
		{
			return CreateGroupingSetsForRollup(mp, grouping_set, num_cols,
											   group_cols, group_col_pos);
		}
		...
		{
			return CreateGroupingSetsForSets(mp, grouping_set, num_cols,
											 group_cols, group_col_pos);
		}
		default:
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
	...
	ForEach(cell, grouping_set->content)
	{
		...
		switch (gs_current->kind)
		{
			...
			case GROUPING_SET_SIMPLE:
				bset = CreateAttnoSetForGroupingSet(mp, gs_current->content,
													num_cols, group_col_pos,
													group_cols, false);
				break;
			default:
											  ...
											  UlongToUlongMap *group_col_pos)
{
	...
	ForEach(lc, grouping_set->content)
	{
		...
		CBitSet *bset =
			CreateAttnoSetForGroupingSet(mp, gs_current->content, num_cols,
										 group_col_pos, group_cols, false);
