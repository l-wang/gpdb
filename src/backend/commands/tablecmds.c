...
static inline void SetSchema(TupleDesc tupleDesc, List **schema, AttrNumber **attnos);
...
static inline void SetSchema(TupleDesc tupleDesc, List **schema, AttrNumber **attnos)
{
		 ...
		 parent_attno++)
	{
		...
		*/
		...
		def->constraints = NIL;
		def->location = -1;
		*attnos[parent_attno - 1] = ++child_attno;
...
void SetSchemaAndConstraints(RangeVar *rangeVar, List **schema, List **constraints)
{
	...
	*/
	...
	if (!pg_class_ownercheck(RelationGetRelid(relation), GetUserId()))
		aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_CLASS, relname);
	...
	SetSchema(tupleDesc, schema, &newattno);
	if (tupleDesc->constr && tupleDesc->constr->num_check > 0)
	{
		SetConstraints(tupleDesc, relname, constraints, newattno);
	}
